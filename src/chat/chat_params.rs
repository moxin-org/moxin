...
impl Widget for ChatParams {
    ...
    fn draw_walk(&mut self, cx: &mut Cx2d, scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        if let Some(chat) = store.chats.get_current_chat() {
            ...
            let context_window = self.slider(id!(context_window));
            ...
            let max = max_context_window(&chat, store);
            context_window.set_range(cx, 512.0, max);
            match chat.context_window {
                ContextWindow::Max => context_window.set_value(max),
                ContextWindow::Custom(value) => context_window.set_value(value.into()),
            }
...
impl WidgetMatchEvent for ChatParams {
    fn handle_actions(&mut self, cx: &mut Cx, actions: &Actions, scope: &mut Scope) {
        ...
        if let Some(chat) = store.chats.get_current_chat() {
            ...
            if let Some(value) = self.slider(id!(context_window)).slided(&actions) {
                chat.context_window = ContextWindow::Custom(value as u32);
            }
        }
    }
}
